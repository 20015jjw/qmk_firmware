...
void keyboard_post_init_user(void) {
  ...
  // Setting the underglow to static, but completely dark
  rgblight_enable_noeeprom();
  rgblight_mode_noeeprom(RGBLIGHT_MODE_REACTIVE);
}
...
void pre_process_action_quantum(keyevent_t event) {
    uprintf("event: row %d col %d ", event.key.row, event.key.col);
    uprintf("time %d\n", event.time);

    if (event.key.row <= 1 && event.key.col <= 1) {
        reactive_led_map[2] = event.pressed;
    }
    else if (event.key.row <= 1 && event.key.col > 1 && event.key.col <= 3) {
        reactive_led_map[1] = event.pressed;
    }
}
